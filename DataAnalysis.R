...
#1st
read <- read.csv("Flights_Delay_Info.csv",stringsAsFactors = FALSE)
Flight_count = read %>% group_by(DayOfWeek) %>% tally

x <- ggplot(Flight_count,aes(x = Flight_count$DayOfWeek, y = n))+ 
  geom_bar(fill = "royalblue4",stat = "identity",width = 0.8)+
  ggtitle("Number of flights on each days") + xlab("Days")+ylab("Count")+
  scale_x_continuous(breaks=seq(1,7,1))

x + theme_economist()+theme(plot.title=element_text(hjust=0.5)) +
